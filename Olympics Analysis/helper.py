...
def athlete_over_time(df,col):
    athlete_over_time = df.drop_duplicates(['Year', 'Name'])['Year'].value_counts().reset_index().sort_index(
        ascending=False)
...
def most_successful_country_wise(df, country):
    ...
    x = temp_df['Name'].value_counts().head(10).reset_index().merge(df, how='left')[['Name', 'count', 'Sport']].drop_duplicates()
